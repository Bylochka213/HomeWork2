...
public class HomeWork2 {
    static class LoginValidationException extends Exception{
        ...
        public LoginValidationException(){
            super("Ошибка!!!! Логин пе прошел проверку на валидность");
        }
        public LoginValidationException(String message){
            super(message);
        }
    }
    //мой метод для проверки строки на валидность
    static boolean test(String str) {
        ...
        if(counter_+counterLetters+counterDigits== myCharArray.length && counter_!=0 && counterLetters!=0&&counterDigits!=0&&myCharArray.length<=20){
            return true;
        }else{
            return false;
        }
